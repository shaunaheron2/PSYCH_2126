...
The first step before analysing a new dataset is to describe and visualize it. **You will want to add a descriptive statistics table and a histogram of your outcome variable in your final document.**

This section is meant as practice, I will list each step needed for your final paper in the "Putting it all Together" tutorial at the very end.

Luckily R has some very handy packages to help us explore our data.

Lets load the iris dataframe and store it in an object we'll call *df*. 

```{r}

df <- data.frame(iris) # load the iris dataframe into an object we'll call df
...
```{r}
descriptives <- describe(df) # create an object out of the output of the describe function

descriptives # this calls the object we just made
```

From the output you can see there are 5 variables, 4 sepal measurements and a species variable. There is no missing data.
...
For a prettier descriptive table of all the continuous variables that includes little histograms:

```{r}

datasummary_skim(df) # a function that shows some descriptives of the numerical data in your df

```

To summarise any categorical variables we can add the type = "categorical" function to our original command. Here we see all of our numeric variables distrbuted across the levels of the categorical (Species).

```{r}

datasummary_skim(df, type = "categorical") # same command, but here we look at our continuous across the levels of Species which is a categorical in the iris dataset

```

Cute!

Obviously there is so much more we can do here, but let's leave it here for the time being.

### Histograms

**The next step after looking at descriptive statistics, is to visualize your data.** 
...
-   the Petal length & Petal width histograms tells us that petal size does NOT follow a normal distribution. Its bimodal, meaning there are two distributions in one. Some petals are really short and some are medium to long. We would have trouble if this were our outcome variable--lucky for us its not.

-   Our outcome variable (sepal length) seems relatively normal.

**Let's have a look at just our outcome variable.** 

In R, a variable name is composed of two parts:

1. the **prefix** (which is the dataframe or table) - df in our case

2. the **variable name** (the exact name the variable has in our df) - Sepal.Length in our case.
...
You will add a histogram of your data to your final document. You'll use the data I give you based on your research design instead of the iris dataset.
...
On quick glance, we can see that sepal length is positively correlated with the Species Virginca (r = .64), meaning that long sepal lengths are more likely in Virginica flowers than Setosa, where sepal length is negatively correlated which suggests the a shorter sepal length.

There is a lot more interesting stuff in the correlation matrix, but to avoid confusion let's move on. Just know that you can build these matrixes in R if you need to. 
